            ...
            <div class="card-body">
              <div class="">
                <table class=" table table-bordered table-striped table-hover datatable desktop" style="width:100%">
                   ...
                   <tbody>
                    @foreach($joining as $key=> $item)
                     <tr>
                        <td>#{{$loop->iteration }}</td>
                        <td>{{$item->name  ?? ''  }}</td>
                        <td>{{$item->mobile_no  ?? ''  }}</td>
                        <td>{{$item->present_address  ?? ''  }}</td>
                        <td>{{$item->find_movement ?? '' }}</td>
                        <td class="text-center ">

                            <a href="{{ route('admin.joining.show',$item->id) }}"class="btn btn-sm btn-outline-primary">
                                <i class="far fa-eye" aria-hidden="true"></i>
                            </a>

                            <a href="{{ route('admin.joining.sendemail',$item->id) }}"class="btn btn-sm btn-outline-primary">
                                <i class="far fa-envelope" aria-hidden="true"></i>
                            </a>

                            <a href="{{ route('admin.joining.destroy',$item->id) }}" class="btn btn-sm btn-outline-danger" id="delete">
